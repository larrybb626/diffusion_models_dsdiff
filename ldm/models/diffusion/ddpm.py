...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.model = DiffusionWrapper_for_other_model(unet_config, conditioning_key)
...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, DiagonalGaussianDistribution) or type(encoder_posterior) == "DiagonalGaussianDistribution":
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        encoder_posterior = encoder_posterior.latent_dist
        encoder_posterior = encoder_posterior.sample()
        z = self.get_first_stage_encoding(encoder_posterior).detach()
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        if self.conditioning_key is None:
            ...
            assert c_adm is not None
            cc = torch.cat(c_crossattn, 1) if c_crossattn[0] is not None else None
...
class DiffusionWrapper_for_other_model(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.sequential_cross_attn = diff_model_config.pop("sequential_crossattn", False)
        self.diffusion_model = instantiate_from_config(diff_model_config)
