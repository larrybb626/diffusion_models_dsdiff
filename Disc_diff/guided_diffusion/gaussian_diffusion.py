...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        if model_kwargs.get("t1") is not None:
            x_input = th.cat([x, model_kwargs["t1"], model_kwargs["t2"], model_kwargs["dwi"]], 1)
        else:
            x_input = x
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        terms = {}
        x_input = th.cat([x_t, model_kwargs["t1"], model_kwargs["t2"], model_kwargs["dwi"]], 1)
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            com_h1, com_h2, com_h3, com_h4, dist_h1, dist_h2, dist_h3, dist_h4, model_output = model(x_input,
            ...
            if com_h3 is not None and dist_h3 is not None:
                ...
                terms['dist'] = mean_flat((dist_h1 - dist_h2) ** 2) + mean_flat((dist_h2 - dist_h3) ** 2) + mean_flat(
                    (dist_h1 - dist_h3) ** 2)
                if com_h4 is not None and dist_h4 is not None:
                    terms['com'] += mean_flat((com_h1 - com_h4) ** 2) + mean_flat((com_h2 - com_h4) ** 2) + mean_flat(
                        (com_h3 - com_h4) ** 2)
                    terms['dist'] += mean_flat((dist_h1 - dist_h4) ** 2) + mean_flat(
                        (dist_h2 - dist_h4) ** 2) + mean_flat((dist_h3 - dist_h4) ** 2)
            else:
