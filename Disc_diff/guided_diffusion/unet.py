...
class UNet_disc_Model(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        self.input_blocks_T2 = nn.ModuleList([copy.deepcopy(module) for module in self.input_blocks])
        self.input_blocks_DWI = nn.ModuleList([copy.deepcopy(module) for module in self.input_blocks])
        ...
        self.dim_reduction_non_zeros = nn.Sequential(
            conv_nd(dims, int(2.5 * conv_ch), conv_ch, 1, padding=0),
            nn.SiLU()
        )
    ...
    def forward(self, x_batch, timesteps):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        # x_batch是多通道的图像，第一个通道是x，第二个通道是T1，第三个通道是T2，第四个通道是DWI，把他们分离出来
        x = x_batch[:, 0:1, :, :]
        T1 = x_batch[:, 1:2, :, :]
        T2 = x_batch[:, 2:3, :, :]
        DWI = x_batch[:, 3:4, :, :]
        h1 = x.type(self.dtype)
        h2 = T1.type(self.dtype)
        h3 = T2.type(self.dtype)
        h4 = DWI.type(self.dtype)
        # print(th.count_nonzero(other))
        # has_other = (th.count_nonzero(other) > 0)

        for idx in range(len(self.input_blocks)):
            h1 = self.input_blocks[idx](h1, emb)
            h2 = self.input_blocks_T1[idx](h2, emb)
            h3 = self.input_blocks_T2[idx](h3, emb)
            h4 = self.input_blocks_DWI[idx](h4, emb)
            hs.append(th.mean(th.stack([h1, h2, h3, h4]), dim=0))

        com_h1 = self.conv_common(h1)
        com_h2 = self.conv_common(h2)
        com_h3 = self.conv_common(h3)
        com_h4 = self.conv_common(h4)
        com_h = self.SE_Attention_com(th.mean(th.stack([com_h1, com_h2, com_h3, com_h4]), dim=0))
        # del com_h1, com_h2, com_h3, com_h4
        ...
        # del com_h, dist_h1, dist_h2, dist_h3, dist_h4
...
class SuperResModelNew(UNet_disc_Model):
    """
    A UNetModel that performs super-resolution.
    Expects an extra kwarg `low_res` to condition on a low-resolution image.
    """
