...
class TryTrainerDiffusion(pl.LightningModule):
    def __init__(self, config):
        ...
        self.test_ds = None
        self.test_num = None
        self.include_test = getattr(config, "include_test", False)
        ...
        val_config.timestep_respacing = str(self.sampler_setting.sample_steps)
        val_config.rescale_timesteps = True
        _, self.diffusion_for_val = sr_create_model_and_diffusion(val_config)
        #######
        if getattr(config, 'use_edge', False):
            if not hasattr(config.unet_config.params, 'use_edge') or not config.unet_config.params.use_edge:
                print(
                    "WARNING: Force setting config.unet_config.params.use_edge to True because config.use_edge is True")
            config.unet_config.params.use_edge = True
        config.unet_config.params.image_size = config.image_size
    ...
    def setup(self, stage: str) -> None:
        ...
        if getattr(self.config, "shannon", False):
            ...
            self.data_mean = self.lowest
            self.data_sd = 0.5
            # self.new_compose = Compose(self.train_transforms.transforms[1:])

    def _extract_batch(self, mean, sd):
        X = get_truncated_normal(mean=mean, sd=sd, low=self.lowest, upp=self.highest)
        stats = Counter(np.round(X.rvs(self.train_batch_size)))
        # # 若stats的键中有8，则随机将另外一个键的值加上8这个键的值，并把8键删除（对部分小数据集）
        # if 8 in stats.keys():
        #     rest_key_list = list(stats.keys())
        #     rest_key_list.remove(8)
        #     key = np.random.choice(rest_key_list)
        #     stats[key] += stats[8]
        #     del stats[8]
        batch_idx_list = []
        for j in stats.keys():
            if stats[j] > len(self.shannon_data_dict[j]):
                stats[j] = len(self.shannon_data_dict[j])
            batch_idx = np.random.choice(self.shannon_data_dict[j], size=stats[j], replace=False)
            for index in batch_idx:
                batch_idx_list.append(index)

        batch = self.shannon_Dataset[batch_idx_list]
        return batch

    def get_warmup_data(self, warm_up_iter):
        batch = self._extract_batch(mean=self.data_mean, sd=self.data_sd)
        ...
        d = {k: to_device(v, self.device) for k, v in batch.items()}

        self.data_mean = self.lowest * (
                    1 - self.global_step / warm_up_iter) + self.highest * self.global_step / warm_up_iter
        return d
    ...
    def do_split(self, K, fold):
        ...
        if self.include_test == True:
            train_id = np.hstack([train_id, test_id])
        # 把train_id 保存到txt
    ...
    def train_dataloader(self):
        if self.train_ds is None:
            raise ValueError("Dataset is None! Check the dataset loading process.")
    ...
    def on_validation_start(self) -> None:
        ...
        self.sample_fn = self.diffusion_for_val.ddim_sample_loop if self.sampler_setting.sampler == 'ddim' else \
            (
                self.diffusion_for_val.dpm_solver_sample_loop if self.sampler_setting.sampler == 'dpm++' else self.diffusion_for_val.p_sample_loop)
        self
    ...
    def on_predict_start(self):
        ...
        if self.sampler_setting.sampler != 'dllm':
            if self.sampler_setting.sample_steps != self.diffusion_for_val.num_timesteps:
                # todo 关键操作，修改config中的timestep_respacing和rescale_timesteps
                self.config.rescale_timesteps = True
                self.config.timestep_respacing = str(self.sampler_setting.sample_steps)
                self.diffusion_for_val = sr_create_model_and_diffusion(self.config)[1]
        self.sample_fn = self.diffusion_for_val.ddim_sample_loop if self.sampler_setting.sampler == 'ddim' else \
            (
                self.diffusion_for_val.dpm_solver_sample_loop if self.sampler_setting.sampler == 'dpm' else self.diffusion_for_val.p_sample_loop)
