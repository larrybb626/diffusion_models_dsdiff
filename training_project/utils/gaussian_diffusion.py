...
from Disc_diff.guided_diffusion.sampler import NoiseScheduleVP, model_wrapper, DPM_Solver
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.parameterization != "v":
            ...
            pred_xstart = self.predict_start_from_z_and_v(x, t, model_output)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, disentangle=None, disen_lambda=0.1):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if isinstance(md_out, tuple):
                model_output, disentangle_dict = md_out
            else:
                model_output = md_out
            ...
            if disentangle:
                hs_list_dict = md_out[1]
                anatomy_feature = th.stack(hs_list_dict['anatomy'], dim=1)  # [[b, c, h, w], ...]->[b, n, c, h, w]
                lesion_feature = th.stack(hs_list_dict['lesion'], dim=1)  # [b, n, c, h, w]
                style_feature = th.stack(hs_list_dict['style'], dim=1)
                content_feature = th.stack(hs_list_dict['content'], dim=1)
                # n_style_content = th.stack(hs_list_dict['n_style_content'],
                #                               dim=1)  # [[h_style, h_n_style, h_share_content, h_n_content]]
                c_s_feature = th.cat([content_feature, style_feature], dim=1)
                a_l_feature = th.cat([anatomy_feature, lesion_feature], dim=1)
                ...
                for b in range(content_feature.shape[0]):
                    c_s_label.append(
                        th.tensor([b] * content_feature.shape[1] + [-1 - j for j in range(style_feature.shape[1])]))
                # c_s_label: [[0,0,0,-1,-2,-3],[1,1,1,-1,-2,-3],...]
                c_s_label = th.stack(c_s_label)

                # todo S-A-L disentangle
                s_a_l_feature = th.cat([style_feature, a_l_feature], dim=1)
                s_a_l_label = []
                ...
                s_a_l_label = th.stack(s_a_l_label)

                # n_style_content_label = []
                # for i in range(n_style_content.shape[0]):
                #     n_style_content_label.append(
                #         th.tensor([-1, -1] + [i] * (n_style_content.shape[1] // 2)))  # [-1,-1, 0,0],[-1,-1,1,1],...
                # n_style_content_label = th.stack(n_style_content_label)
                contrast = disentangle
                ...
                perfect_s_a_l_heatmap = get_heatmap(perfect_logit)
                terms['contrast_map'] = {'c_s_heatmap': c_s_heatmap,
                                         'perfect_c_s_heatmap': perfect_c_s_heatmap,
                                         's_a_l_heatmap': s_a_l_heatmap,
                                         'perfect_s_a_l_heatmap': perfect_s_a_l_heatmap}
                terms['disen_c_s_loss'] = content_style_loss
                terms['disen_s_a_l_loss'] = style_anatomy_lesion_loss
    ...
    def get_disentangle_loss(self, feature, label, contrast='eu', temperature=0.1):
        ...
        if contrast == 'contrast':
            ...
            label = th.cat(th.unbind(label, dim=1), dim=0)
            label = label.contiguous().view(-1, 1).to(feature.device)
            feature = th.cat(th.unbind(feature, dim=1), dim=0)
            feature = feature.view(feature.shape[0], -1)
            feature_64 = feature.double()
            logits = th.cdist(feature_64, feature_64, p=2)
            logits = logits.float() / feature.shape[1]
            mask = th.eq(label, label.T)
            # perfect_logit = th.where(mask == 1, th.tensor(1.0), th.tensor(-1.0))
            perfect_logit = 2 * mask.float() - 1
            eye_mask = th.eye(label.shape[0], dtype=th.bool).to(feature.device)
            numerator = (logits * ~eye_mask * mask).sum()
            denominator = (logits * ~mask).sum()
            loss = th.div(numerator, denominator).float()
            return loss, logits * 2 - 1, perfect_logit
        elif contrast == 'eu&contrast':
            loss_con, logits_con, perfect_logit_con = self.get_contrastive_loss(feature, label, temperature=temperature)
            label = th.cat(th.unbind(label, dim=1), dim=0)
            label = label.contiguous().view(-1, 1).to(feature.device)
            feature = th.cat(th.unbind(feature, dim=1), dim=0)
            feature = feature.view(feature.shape[0], -1)
            logits = th.cdist(feature, feature, p=2)
            ...
            numerator = (logits * ~eye_mask * mask).sum()
            denominator = (logits * ~mask).sum()
            loss = th.div(numerator, denominator)

            return loss + 0.05 * loss_con, logits_con, perfect_logit_con
        else:
            raise NotImplementedError(f"contrast {contrast} not supported")
