...
from training_project.utils.util import save_tensor_hdf5
...
class DSUnetModel(nn.Module):
    def __init__(self,
        ...
        # )
        # condition with emb
        # self.linear_group = nn.ModuleList([nn.Linear(half_conv_ch*image_size//32, 384) for _ in range(4)])
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        for i in range(len(context)):
            ...
            # 另类的方式输入
            # context[i] = context[i].view(context[i].size(0), -1)
            # context[i] = self.linear_group[i](context[i])


        # len_highres_context = len(self.context_dim_highres)
        lowres_context = context[0:2]
        highres_context = context[2:]
        # lowres_context = context[1:]
        # highres_context = context[0:1]
        lowres_context = None
        highres_context = None
        for module in self.input_blocks:
            h_n = module(h_n, emb, highres_context)
            hs.append(h_n)
        h_n = self.middle_block(h_n, emb, None)
        if max(timesteps) == 0:
        ...
        for module in self.output_blocks:
            ...
            h = module(h, emb, highres_context)
        h = h.type(x.dtype)
        if self.predict_codebook_ids:
