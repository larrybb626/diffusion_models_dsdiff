...
def process_id(id_num, real_dir, whole_dir, output_dir, exp_name, labels):
    ...
    # 读取两张 NIfTI 图像
    nii_path_gt = os.path.join(real_dir, id_num, "ce.nii.gz")
    ...
    for method in methods:
        ...
        diff_images_list = [compute_difference(A, model_img, method) for model_img in nparray_list]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--real_dir', type=str,
                        default="/data/newnas_1/MJY_file/BraTS_dataset/data_pre_original/images_ts")
    parser.add_argument('--fake_dir', type=str,
                        default="/data/newnas_1/MJY_file/diffusion/train_result_BraTs")
    parser.add_argument('--output_dir', type=str, default="/data/newnas_1/MJY_file/visualize_all/differmap_brats_p4")
    parser.add_argument('--id_num', type=str, default="0000894408")

    args = parser.parse_args()
    real_dir = args.real_dir
    whole_dir = args.fake_dir
    id_list = os.listdir(real_dir)
    # labels = [
    ...
    num_workers = min(16, len(id_list))  # 限制进程数
